...
void Encrypt(param par, Fr& s, int i, std::string& m, cipher Ci, Fp12& Caid )
{
    ...
    Fp12::div(Ci.C3, e1, e2); // C3 = e(H(m), g_2)^t / e(g_1, g_i)^t
    Fp12::div(Caid, e1, e2);  // Caid = e(H(m), g_2)^t / e(g_1, g_i)^t
...
int main()
{
    ...
    i= 1;
    for (i=0; i < n; i++){
        Trapdoor(Tr[i], raid[i], pubk, kagg, S, msgVec[i]);
    }
    ...
    j=0;
    for (i = 0; i< n; i++){
        Adjust (trapi[i], pubk, raid[j], Tr[j], i, S);
    }
    
    //Test
    j = 3;
    for (i=0; i <n; i++){
        bool hit = Test (pubk, cil[i], Caid[i], Tr[j], raid[j], trapi[j], S);
